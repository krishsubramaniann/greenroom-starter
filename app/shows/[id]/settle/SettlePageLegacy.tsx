/**
 * Legacy settle page — the implementation that shipped before the V2 engine.
 *
 * Routes here when `deal.confirmedAt` is null (i.e., the deal has not been
 * captured through the V2 capture flow). Preserved verbatim from the prior
 * `page.tsx`; the function signature is renamed to a named export taking a
 * pre-loaded `data` object so the new thin router in `page.tsx` can dispatch
 * between V1 and V2.
 */
...
import { getShowById, type ShowWithRelations } from "@/lib/queries";
import {
  Card,
  CardContent,
  CardHeader,
...
export async function SettlePageLegacy({ data }: { data: ShowWithRelations }) {
